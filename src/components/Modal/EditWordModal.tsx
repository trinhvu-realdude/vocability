...
export const EditWordModal: React.FC<WordFormProps> = ({
    db,
    word,
    collection,
    setIsEditOrDelete,
    ...
    useState(() => {
        // Use a small timeout to ensure the browser registers the state change for transition
        setTimeout(() => setShowModal(true), 10);
    });

    const handleClose = () => {
        setShowModal(false);
        setTimeout(() => setIsEditOrDelete(false), 150); // Wait for transition
    };
    ...
    return (
        <div
            ...
            tabIndex={-1}
        >
            <div className={`modal-dialog modal-lg modal-dialog-centered ${showModal ? 'modal-dialog-scrollable' : ''}`}>
